...
evaluation), and if re-entering an earlier phase applies, include the \
key "reentrada" with the value "fase3" (the purpose no longer resonates) \
or "fase4" (the system needs a redesign); if no re-entry is needed, \
omit that key or leave it null."""
...
def construir_vista_resumen(ficha_actual: dict | None, idioma: str = "es") -> str:
    if idioma == "en":
        if not ficha_actual:
            return "(no ficha on record yet)"

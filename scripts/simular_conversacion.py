...
Cada turno imprime, además del texto de la respuesta:
  ...
  - si la fase realmente avanzó según la ficha (AgentCore Memory) -- si
    "declaró cerrado" es True pero la fase no avanzó, es una alerta
    directa de que el bug de cierre falso reapareció y hubo que forzar
    un reintento (_verificar_y_reforzar ya lo corrigió, pero vale la
    pena saber que pasó)
...
def _imprimir_turno(sesion: SesionTelos, fase_antes: int, texto: str, opciones: list[str]) -> None:
    ...
    fase_invocada = sesion.ultima_fase_respondio
    cerrado_declarado = sesion.ultimo_cerrado_declarado

    print(f"\n[{nombre_antes}{cambio}] {texto}")
    if opciones:
        print(f"   opciones ofrecidas: {opciones}")
    if fase_invocada is not None:
        nombre_invocada = _NOMBRES_FASE.get(fase_invocada, fase_invocada)
        if fase_invocada != fase_antes:
            print(f"   !!! ALERTA: el orquestador invocó {nombre_invocada}, no la fase esperada ({nombre_antes})")
        print(f"   cerrado declarado por {nombre_invocada}: {cerrado_declarado}")
        if cerrado_declarado and fase_despues == fase_antes:
            print("   !!! ALERTA: declaró cerrado=True pero la fase NO avanzó (cierre falso, ver _verificar_y_reforzar)")
...
def main() -> None:
    ...
    for fase, texto, opciones in sesion.abrir_conversacion():
        _imprimir_turno(sesion, fase_previa, texto, opciones)
        fase_previa = sesion.fase_actual
